...
- [x] **T013** Implement `src/fetcher/parser.ts`: Cheerio HTML → ScheduleEntry[], empty/selector-mismatch → `[]` + warn | fetcher | [T004] | Spec: fetcher FR-parse | ~30min
- [x] **T014** Record HTML fixtures: `__fixtures__/schedule-single-day.html`, `schedule-empty.html`, `schedule-malformed.html` | fetcher | [T013] | Spec: fetcher scenarios | ~15min
- [x] **T015** Unit tests: `fetcher/client.test.ts` (nock-recorded retries, timeout), `fetcher/parser.test.ts` (3 fixtures) | fetcher | [T012, T013, T014] | Spec: fetcher FR, NFR | ~30min
